...
class WDFReader(object):
    ...
    def convert_spectra(self, new_spectra,  start=0, end=-1):
        """Get information from DATA block
        """
        if end == -1:           # take all spectra
            end = self.count - 1
        ...
        if start > end:
            raise ValueError("Start cannot be larger than end!")
        n_row = end - start + 1
        # Check the new spectra's shape
        if new_spectra.shape != (n_row, self.point_per_spectrum):
            raise ValueError("New spectra has incorrect shape!")
        # Determine start position
        uid, pos, size = self.block_info["DATA"]
        pos_start = pos + Offsets.block_data + LenType["l_float"].value * \
            start * self.point_per_spectrum
        ...
        # transform the new_spectra
        new_spectra = new_spectra.flatten()     # convert new_spectra to 1D array
        new_spectra = struct.pack('f'*len(new_spectra), *new_spectra)     # convert new_spectra to binary format
        self.file_obj.write(new_spectra)
        return
